...
1.  First we will make a basic histogram of the Avg_DOY_SpringFrost
    column. You can do this using this code. To understand what is going
    on, see this great tutorial:
...
2.  We can improve it very easily. Try running this code.
...
3.  Now make your own histogram of the Latitude column of the frost
    data. Go wild, impress me with your formatting!
...
1.  First, we loaded our data as a standard old Excel file. So we need
    to teach R that it has spatial coordinates. To do this, run this
    command. If it works, nothing should outwardly happen, but you
    should see in your environment quadrant that a new variable called
    frost.spatial has turned up.
...
2.  Now we can use the QTM (Quick Thematic Map function) to have a look!
    This should make an interactive map by default and you can turn on
    different basemaps.

    ```{r,eval=FALSE}
    # This sets the mode to an interactive map. If your computer struggles, try changing 'view' to 'plot'
    tmap_mode('view')
...
3.  Try making your own map of the Avg_DOY_SpringFrost column in the
...
This section will be added in the next hour, but really use next week to
focus on it.
