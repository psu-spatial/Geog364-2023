...
1.  Make a new code chunk. Inside, calculate the mean of the
    Dist_To_Coast column. e.g. `mean(frost$Dist_To_Coast)`
...
4.  We can also apply commands to non-numeric data. For example, maybe
    ...
    table command: `table(frost$State)` How many weather stations are in
    Alabama?
...
For plots, there are *many* packages you could use. Today we are going
to focus on the ggplot2 and the tmap packages.

To be uploaded in the next 10mins. REFRESH THE PAGE!
