...
3.  Google the command for the standard deviation in R. Find the
    standard deviation of the distance of the rain-gauges to the coast.

4.  We can also apply commands to non-numeric data. Use the `table`
    command on the State column e.g. `table(frost$State)` How many
    rain-gauges are in Alabama?

5.  R includes some fantastic summary commands that we can use on the
    whole table. Run `summary(frost)` and explain what is going on.

## 9: Intro plots

Uploaded in the next 10mins REFRESH THE PAGE
