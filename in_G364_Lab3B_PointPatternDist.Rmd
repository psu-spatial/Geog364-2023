...
# Hint, your ppp variable is called *market.ppp*. 
 longleaf.nnd <- nndist(longleaf)

#now I make a histogram
 hist(longleaf.nnd,br=50)
```
...
#-------------------------------------
 paste( c("The average nearest neighbour of my longleaf data is:"),
        round(mean(longleaf.nnd),3),"m away")
```
...
longleaf.obs_mean_nnd  <- mean(longleaf.nnd) 
...
# 8. Calculate the Clarke-Evans Ratio, R
...
# 9. Conduct a hypothesis test on it
...
2.  Get the code in tutorials 6-8 working for your market data (hint,
    remember to remove any reference longleaf!)<br><br>

3.  Now explain what the output of the code is showing you for the
    nearest neighbour and Clarke Evans Ratio. What units are the
    distances in? (hint, google the units of your new map
    projection)<br>
...
4.  Now write your interpretation of the output.<br>
    ...
    -   E.g. this show the data is clustered, random or dispersed?<br>
...
# THIS IS BEING UPLOADED IN PARTS - IF YOU SEE THIS BANNER PLEASE REFRESH THE PAGE AFTER 2.15pm TUESDAY

# 11. Ripleys K hypothesis test

Finally, we can assess the significance of this result using a
hypothesis test. Here I show you how to conduct a 'pseudo test' using
simulations. E.g. we directly create a load of H0 scenarios, calculate
the clarke evans ratio for each and see the probability of seeing our
result

```{r, eval=FALSE,echo=FALSE}

# Here I am creating 10 point patterns with complete spatial randomness, and for each one calculating the clarke evans ratio
longleaf.RRatio.simulations <- envelope(longleaf, 
                                        fun = clarkevans, 
                                        nsim = 10)
...
# Compare the observed Clarke-Evans Ratio to the envelope
longleaf.obs.RRatio
summary(envelope_result)

# Check if the observed value is significantly different
p_value <- ppp.test(envelope_result, longleaf.obs.RRatio)
```
